...
{  
    ...
    glRotatef(*tetha1*RTD,0,0,1); //Untuk memutar link selanjutnya
    glRotatef(*tetha2*RTD,1,0,0); //sudut,x,y,z
    
    /* Gambar Link-1 */
    glTranslatef(0,0, Link1/2);
    glPushMatrix();
    model_cylinder(obj, 0.05, 0.05, Link1, 2, blue1, yellow2);
    penunjuk();
    glPopMatrix();
    ...
    /* Menuju Joint-4 */
    glTranslatef(0,0, Link2/2);
    glRotatef((90-(*tetha3+*tetha2)*RTD), 1,0,0);
    glRotatef(*tetha4*RTD, 0,1,0);
    
    /* Gambar Link-3 */
    glTranslatef(0, 0, Link3/2);
    glPushMatrix();
    model_cylinder(obj, 0.05, 0.05, Link3, 2, yellow5, yellow2);
    penunjuk();
    glPopMatrix();
...
{
  static int mode = Naik;
  if(mode==Naik) q1 = 0 + 50*DTR * t/5;
  else q1 = 50 - 50*DTR * t/5;
}
...
void keyboard(unsigned char key, int i, int j)
{
  switch(key)
  {
    ...
    case 'A': q1 += -10*DTR; break;
    case 's': q2 += 10*DTR; break;
    case 'S': q2 += -10*DTR; break;
    case 'd': q3 += 10*DTR; break;
    case 'D': q3 += -10*DTR; break;
    case 'f': q4 += 10*DTR; break;
    case 'F': q4 += -10*DTR; break;
...
int main(int argc, char** argv)
{
 /* Initialize GLUT */
   ...
   /* Open a window */  
   window = glutCreateWindow ("4 DOF Robot"); // Bikin window dan judul
